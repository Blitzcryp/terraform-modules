...
run "secure_defaults" {
  ...
  # The instance atom is composed alongside one subnet group and SG; no parameter
  # group when no parameters are supplied.
  assert {
  ...
  assert {
    condition     = length(module.parameter_group) == 0
    error_message = "No parameter group must be created when no parameters are supplied."
  }

  # Engine-derived DB port for postgres.
  assert {
  ...
  assert {
    condition     = can(output.manifest.instance_id) && can(output.manifest.instance_arn) && can(output.manifest.endpoint) && can(output.manifest.address) && can(output.manifest.port) && can(output.manifest.security_group_id) && can(output.manifest.master_user_secret_arn) && can(output.manifest.kms_key_arn)
...
run "parameters_create_parameter_group" {
  command = plan
  ...
  assert {
    condition     = length(module.parameter_group) == 1
    error_message = "A parameter group must be created when parameters are supplied."
  }
}
