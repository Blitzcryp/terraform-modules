locals {
  module_tags = {
    Module = "components/rds-aurora-serverless" # only hardcoded tag; global tags come from provider default_tags
  }
...
module "kms" {
  ...
  config = {
    description = "Encrypts the ${var.config.name} Aurora Serverless v2 cluster at rest"
...
module "db_subnet_group" {
  ...
  config = {
    ...
    description = "Subnet group for the ${var.config.name} Aurora Serverless v2 cluster"
    tags        = local.tags
  }
}
...
module "security_group" {
  ...
  config = {
    ...
    description   = "DB-port access for the ${var.config.name} Aurora Serverless v2 cluster"
    ingress_rules = local.ingress_rules
    tags          = local.tags
  }
}

# --- Aurora Serverless v2 cluster (secure by default) -------------------------
#
# Setting serverlessv2_scaling_configuration is what makes this the serverless
# preset: the cluster atom then forces every instance onto db.serverless and
# scales between min/max ACUs. Everything else matches the provisioned preset.
