variable "config" {
  description = <<-EOT
    Configuration for the rds-aurora-serverless component (a secure-by-default
    Aurora Serverless v2 cluster). Same composition and config as the rds-aurora
    component, but instead of instance_class it exposes min_capacity / max_capacity
    (Aurora Capacity Units) and forces instances onto db.serverless via the cluster
    atom's serverlessv2_scaling_configuration. All inputs live on this single
    object. PCI-DSS-compliant defaults are baked into the optional() fields:
    storage is encrypted, deletion protection is on, backups are retained, IAM
  ...
  type = object({
    ...
    # --- Engine / sizing (Serverless v2 capacity in ACUs) ---
  ...
  validation {
    condition     = var.config.min_capacity >= 0.5 && var.config.max_capacity <= 256 && var.config.min_capacity <= var.config.max_capacity
    error_message = "config.min_capacity/max_capacity must be within 0.5..256 ACUs and min_capacity <= max_capacity."
  }
